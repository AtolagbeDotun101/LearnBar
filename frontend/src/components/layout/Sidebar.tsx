      ...
      ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full '}`}>
        ...
        <div className='flex items-center justify-between h-16 px-5 border-b border-slate-200/60'>
          <div className='flex items-center gap-3'>
            <div className='flex items-center justify-center w-9 h-9 rounded-xl bg-linear-to-br from-emerald-400 to-teal-500 shadow-md shadow-emerald-500/20'>
              <BrainCircuit className='text-white' strokeWidth={2.5} size={20} />
            </div>
        ...
        <nav className='flex-1 px-3 py-6 space-y-1.5 '>
         {navLinks.map((link: any)=>(
          ...
          to={link.to}
          onClick={toggleSidebar}
          className={({isActive}) => `group flex items-center gap-3 py-2.5 px-4 text-sm font-semibold rounded-xl transition-all duration-200
          ${isActive ?
            'bg-linear-to-r from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/25 bg-white'
        ...
        <div className='px-3 py-4 border-t border-slate-200/60'>
          <button className='group flex items-center gap-3 w-full px-4 py-2.5 text-sm font-semibold text-slate-700 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-200' onClick={handleLogout}>
            <LogOut
            size={18}
            strokeWidth={2.5}
