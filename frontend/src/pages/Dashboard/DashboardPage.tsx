      ...
      <div className='absolute inset-0 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] opacity-30 pointer-events-none' />
        <div className='relative max-w-7xl mx-auto'>
          ...
          <div className='mb-6'>
            ...
            <p className='text-slate-500 text-sm'>Track your learning progress and activities</p>
          </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-5'>
            {stats.map((stat, index) =>(
              <div
              key={index}
              className='group relative bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-6 hover:shadow-2xl hover:shadow-slate-300/50 transition-shadow duration-300 hover:translate-y-1'
              >
          ...
          <div className='bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-8'>
            ...
            {dashboardData.recentActivity && (dashboardData.recentActivity.documents.length > 0 || dashboardData.recentActivity.quizzes.length > 0) ? (
             <div className=''>
              {[...(dashboardData.recentActivity.documents || []).map((doc:any) =>({
                id: doc._id,
            ...
            .map((activity, index) => (
              ...
              className=''
              >
                <div className='group flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-200/60 hover:bg-white hover:border-slate-300/60 hover:shadow-md transition-all duration-200'>
                  <div className='flex items-center gap-2 mb-1'>
                    <div className={`w-2 h-2 rounded-full  ${
                      ...
                      <p className='text-sm font-medium text-slate-900 truncate'>{activity.type === 'document' ? 'Acessed Document:' : 'Attempted Quiz'}
                        <span className='text-slate-700'> {activity.description}</span>
                      </p>
                    </div>
                    <p className='text-slate-500 pl-4 text-xs'>{new Date(activity.timestamp).toLocaleString()}</p>
                  </div>
                  {activity.link && (
