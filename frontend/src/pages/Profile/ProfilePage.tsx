...
const ProfilePage = () => {
  ...
  const handleChangePassword = async(e:any) =>{
    ...
    if(newPassword !== confirmNewPassword){
      toast.error('New passwords do no not match')
      return;
    }
    if(newPassword.length < 6){
      toast.error('New passwords must be at least 6 characters long.')
      return;
    }
    setPasswordLoading(true)
    try {
      await authService.changePassword(confirmNewPassword, newPassword);
