...
const renderContent = ()=> {
  ...
  return (
    <div className='min-h-screen'>
      ...
      {isUploadModalOpen && (<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop:backdrop-blur-sm'>
         <div className='relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
          ...
          className='absolute top-6 right-6 w-8 h-8 flex items-center rounded-lg text-slate-600 hover:text-slate-300 transition-all duration-200'
          >
          ...
          <form className='space-y-5' onSubmit={handleUpload}>
            <div className='space-y-2'>
              ...
              required
              className='w-full h-12 px-4 border-slate-200 rounded-xl bg-slate-50/55 text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:outline-none font-medium focus:shadow-lg focus:shadow-emerald-500/10 transition-all duration-200'
            ...
            <div className='space-y-2'>
              ...
              <div className='relative border-2 border-dashed border-slate-50/50 rounded-xl  hover:border-emerald-400 transition-all duration-200'>
                <input type="file"
                id='file-upload'
            ...
            <div className='flex gap-3 pt-2'>
              ...
              onClick={()=> setIsUploadModalOpen(false)}
              disabled={uploading}
              className='flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-emerald-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'
              >Cancel</button>
               <button
               type='submit'
                disabled={uploading}
                className='flex-1 h-11 px-4 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-sm font-semibold rounded-xl shadow-lg shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/30 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed'>
                  {uploading? (
    ...
    {isDeleteModalOpen && (<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop:backdrop-blur-sm'>
      <div className='relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
        {/* Close button */}
        <button className='absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 transition-all duration-200'
        onClick={()=> setIsDeleteModalOpen(false)}
        ><X className='w-5 h-5'  strokeWidth={2}/></button>
        {/* Modal Header */}
        <div className='mb-6'>
        ...
        <div className='flex gap-3'>
          <button className='flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-red-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'
          type='button'
          disabled={deleting}
          onClick={()=> setIsDeleteModalOpen(false)}
          >
          ...
          <button className='flex-1 h-11 px-4 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-xl shadow-lg shadow-red-500/25 hover:shadow-xl hover:shadow-red-500/30 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed'
          disabled={deleting}
