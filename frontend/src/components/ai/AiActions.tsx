...
const AiActions = () => {
  ...
  const handleExplainConcept = async (e: any) => {
    e.preventDefault();
    if (!concept.trim()) {
    ...
    <>
      <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          {/* Generate Summary */}
          <div className="p-5 group bg-linear-to-br from-slate-50/50  to-white rounded-xl border border-slate-200/60 hover:border-slate-300/60 hover:shadow-md transition-all duration-200">
          ...
          <div className="group p-5 bg-linear-to-br from-slate-50/50 to-white rounded-xl border border-slate-200/60 hover:border-slate-300/60 hover:shadow-md transition-all duration-200">
            <form onSubmit={handleExplainConcept}>
                ...
                <p className="font-semibold text-slate-600 leading-relaxed mb-4">Enter a topic or concept from the document to get a detailed explanation.</p>
                <div className="flex items-center gap-3">
                    <input type="text"
                    value={concept}
                    ...
                    disabled={loadingAction === 'expalin'}/>
                    <button 
                    type="submit"
                    disabled={loadingAction === 'explain' || !concept.trim()}
                    className="shrinl-0 h-11 px-5 bg-linear-to-rn from-emerald-600 to-emerald-500 hover:from-emerald-600 hover:to-emerald-600 text-white text-sm font-semibold rounded-xl transition-all duration-200"
                    >
