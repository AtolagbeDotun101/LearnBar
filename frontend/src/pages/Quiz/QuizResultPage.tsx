...
const QuizResultPage = () => {
  ...
  const correctAnswer = detailedResults.filter((r:any)=> r.isCorrect).length;
    ...
    <div className='max-w-5xl mx-auto'>
      ...
      <div className='space-y-6'>
        ...
        {detailedResults.map((result:any , index: number)=>{
          const userAnswerIndex = result.options.findIndex((opt:any) => opt === result.selectedAnswer);
          const correctAnswerIndex = result.correctAnswer.startsWith('o')
          ? parseInt(result.correctAnswer.subString(1)) - 1
          : result.options.findIndex((opt:any)=> opt === result.correctAnswer);
          ...
          return(
            <div
            key={index}
            className='bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl p-6 shadow-lg drop-shadow-slate-200/50'>
              ...
              <div className='space-y-3 mb-4'>
                {result.options.map((option:any, optIndex:any)=>{
      ...
      {/* Action Button */}
      <div className='mt-8 flec justify-center'>
        <Link
        to={`/documents/${quiz.document._id}`}>
          <button className='group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden'>
            ...
            <div className='absolute inset-0 bg-linear-to-r from-white/0 via-white/20 to-white/20 -translate-x-full group-hover:translate-x-full transition-transform duration-7 00'/>
          </button>
        </Link>
      </div>
    </div>
